...
/**
 * Clase que amacena la lectura de un bloque de bytes para trabajar con el en
 * memoria
 *
 * @author César Pomar
 */
public final class Buffer {
    ...
    /**
     * Mira los siguientes bytes sin avanzar la lectura
     *
     * @param bytes numero de bytes
     * @return array de bytes
     */
    public byte[] checkBytes(int bytes) {
        islimit(bytes);
        byte[] array = new byte[bytes];
        for (int i = 0; i < bytes; i++) {
            array[i] = buffer[posicion + i];
        }
        return array;
    }
     ...
     *
     * @param n salto
     * @return Numero de bytes saltados
     */
    public int skipM4(int n) {
